...
%          - normalised likelihood for all Np+1 non degenerate models evaluated at the maximum likelihood point
%          - penalisations (BIC,AIC,MDLl, MDLu,ENT,COUNTPATTERNS see paper).
%          The penalizations come in a structure form as a second output and each structure element is an array of length Np+1
...
% Binary representation of the model: this is a number of 2^Np bits and each bits tells you if the correspondent operator (parameter) is on or off
model = ones(Np+1,Np);
