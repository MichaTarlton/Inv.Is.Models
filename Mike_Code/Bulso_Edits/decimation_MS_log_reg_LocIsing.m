...
vNp_inactive = round(linspace(0,Nx,6)); 			% Creates linear vector of 6 points evenly spaced bt 0 and Nx, each rounded to nearest integer
for iNp_inactive = 1:numel(vNp_inactive)			% Purpose of iNp_inactive creates number of iterations the for loop will run based on number of point in vNP (Isn't it preset to 6 #?)
    Np_inactive = vNp_inactive(iNp_inactive);		% Selects the value from linear vector for current cycle of loop
    sparsity = Np_inactive/Np;						% Sparsity? what is this value used for? Ah, as stated at the top, gives the fraction of active couplings, so number active out of total parameters
    
    for vb = 1:length(vbeta)						%Runs loop for every element of vbeta "input features"
        

        %% tuning parameter localisation of inputs distribution inverse temperature Ising ferromagnetic
        
        beta = vbeta(vb);							% Sets beta to current (local) beta value
        
        %% input nodes distributed as Ferromagnetic Ising model with temperature 1/beta
        xsi = ones(1,Nx);							% | Not sure what this is, other than a vector of ones the size of number of nodes #?
        W = xsi'*xsi; 								% hebbian learning to store the config in the connectivity | Transpose of the vector mulitplied by itself to create an array (possibly replaced by ones(Nx) #?)
        W = W - diag(diag(W)); 						% no self interactions () | Removes the diagonal from the array so that there is 
        W = W/Nx; 									% so effective field is O(1) and comparable with beta | Don't get this exactly
        epsil = 0.1;								% | epsilon #? Weak Field?
        H = epsil*xsi; 								% weak field order 0.1 to break the symmetry | H vector created by multiplying ones vector by 0.1 #?
        
        %% initializations							% Why are we creating all these zero arrays? it's just storage optimization. I guess I need to know what the arrays actually are, but need to read further down where they are populated
        ...
        Fp.BIC = zeros(numel(M),1);
        Fn.BIC = zeros(numel(M),1);Err.BIC = zeros(numel(M),1);
        BestModel.BIC = zeros(numel(M),Np);
        CostBest.BIC = zeros(numel(M),1);
        Posterior.BIC = zeros(Np+1,numel(M));
        
        Fp.AIC = zeros(numel(M),1);
        Fn.AIC = zeros(numel(M),1);
        Err.AIC = zeros(numel(M),1);
        BestModel.AIC = zeros(numel(M),Np);
        CostBest.AIC = zeros(numel(M),1);
        Posterior.AIC = zeros(Np+1,numel(M));
        
        Fp.MDLl = zeros(numel(M),1);
        Fn.MDLl = zeros(numel(M),1);
        Err.MDLl = zeros(numel(M),1);
        BestModel.MDLl = zeros(numel(M),Np);
        CostBest.MDLl = zeros(numel(M),1);
        ...
        if d ~= 0
            w = w/sqrt(d);                              % Just doing a simple normalization
        end
