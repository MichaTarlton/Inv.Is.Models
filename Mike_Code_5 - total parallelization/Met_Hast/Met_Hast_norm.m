...
function SstructNorm = Met_Hast_norm(T,N,J,h,sparsity,time,beta)

% in this case the jn will be the topologies we are comparing and not the trials. THis whole thing will generate multiple topological S outputs for a single trial 	
...
%parfor i = 1:jn
	...
	SstructNorm.S_hat    = S_hat;
	SstructNorm.mi   	= mi;
	SstructNorm.mimj    	= mimj; 
	SstructNorm.chi 		= chi;
	SstructNorm.Cij   	= Cij;
	SstructNorm.mCij 	= mCij; 
	SstructNorm.mequil   = mean(S_eq);
	SstructNorm.Chiequil = S_eq'*S_eq/T;
