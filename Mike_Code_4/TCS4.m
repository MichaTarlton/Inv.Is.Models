...
function [Adj,J,hfield] = TCS4(topology,N,c,couplings,beta,J0,sprs,h_on)
...
%% Topology and Sparseness
	...
	elseif topology == 7
		...
		graphy = WattsStrogatz(N,ceil(c./2),p) %this is passedout in matlab graph format. I need to convert to array
...
%%coupling strength
	...
	elseif couplings == 4 %| Mike's gaussian from the "SK" model gen: JHnorm, difference is mine is meaned around 0
		...
		%distr = sigJ;
		J0 = 0;
% turned off for jgraphs chk		R = normrnd(J0,distr,[N,N]);
		R = (-beta + (beta+beta)*rand(1,N)); % THe N17 check field
		R = triu(R,1) + triu(R,1)';
		J = beta.*Adj.*R;
	...
	elseif couplings == 5 % My own take on Nicola's gaussian
	 	...
	 	J0 = 1; % We always assume the mean is 1, then multiplied by beta or "Distr"
	 	%distr = sigJ;
	 	
	 	R = normrnd(J0,distr,[N,N]) 	% Mean at J0 (typically 1) with std of distr in a matrix NxN
	 	negs = ones(N) 				% Create NxN ones matrix
	 	negs(rand(N) > 0.5 ) = -1		% make random half of values of ones negative
	 	R = R.*negs					%  Multiply half negs against our mean, making it a split mean 
	 	R = triu(R,1) + triu(R,1)'		% Symmetry
	 	J = distr.*beta.*Adj.*R		% adjacency, beta , and normalization for final J matrix 	
...
if h_on == 1
	...
	hfield = beta*(-0.3 + (0.3+0.3)*rand(1,N)); % THe N17 check field
