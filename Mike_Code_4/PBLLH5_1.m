%PBLH5_1
% this one can save in progress stats so we don't lost unsaved partial overstructs


function [LLH,statvecs,stats,jta] = PBLLH5(T,N,tp,beta,c,h_on,SStruct,JHnorm,jta,jtatot,topdir,intbeta)
...
lm = length(modelvec);
statfieldnames  = [{'symrate'},{'asymrate'},{'totconerr'},{'perconerr'},{'tnconerr'},{'tpconerr'},{'fnconerr'},{'fpconerr'},{'tnpererr'},{'tppererr'},{'fnpererr'},{'fppererr'},{'fallout'},{'recall'},{'precision'},{'TFS'},{'TFR'},{'TFN'},{'NPR'},{'FOR'},{'FDR'},{'symtotconerr'},{'symperconerr'},{'symtotconsererr'},{'symperconsererr'},{'Jrrerr'},{'Jsymrrerr'},{'Jsymconserrrerr'},{'Javgsymrrerr'}];
...
for st = 1:size(SStruct,2) % Structs make sizing weird, this is correct though
	...
	NodeModel = struct('w_ML',{},'l_ML',{},'posterior',{},'cost',{},'BestModel',{},'IMAX',{});	
	 ...
	 thetaMDLcount = [];
		...
		parfor t = 1:N
			...
			disp(['Sprs ', num2str(c),' Topo ', num2str(st),' N ', num2str(N),' T ', num2str(T), ' Node ', num2str(t)])
	...
	for mt = 1:lm
		...
		LLH(st).tnpererr.(model)      =  LLH(st).tnconerr.(model) ./ N.^2;
		% True positives percent 
		LLH(st).tppererr.(model)      =  LLH(st).tpconerr.(model) ./ N.^2;
		% False Negatives percent  
		LLH(st).fnpererr.(model)      =  LLH(st).fnconerr.(model) ./ N.^2;
		% False positives percent  
		LLH(st).fppererr.(model)      =  LLH(st).fpconerr.(model) ./ N.^2;
