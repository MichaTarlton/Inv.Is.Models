...
function [LLH,statvecs,stats,jta] = PBLLH4(T,N,tp,beta,sprs,h_on,SStruct,JHnorm,jta,jtatot)
...
for st = parsteps(1:size(SStruct,2)) % Structs make sizing weird, this is correct though
...
for mt2 = 1:lm 
	...
	for ft2 = 1:fm
		...
		stats.stderr.(model).(statfieldnames{ft2}) =	[std(statvecs.(model).(statfieldnames{ft2}))/sqrt(size(S,2))]; % std(x)/sqrt(length(x));
