...
%		3. AUPRC v beta
%		4. rrerr v beta
%		4. Perconerr v beta for a symmetric array
%		4. PPC v beta
% Ideally we can change up the x-axis
% I want to be able to do something more like Fig 1 from bento

function [newstruct,figstor,dupes] = multiallgraph(multiallstruct,comp,topdir,time)
...
toponamevec = [{'Cayley Tree'},{'Independent Pair'},{'2D Ising'},{'Erdős–Rényi'},{'Star Netowrk'},];
topologies = [1,3,4,5,6]; %all for now
				...
				for topn = 1:ltop
					...
					for bn = 1:lub % number of beta values & we don't need another loop because the values of the subplots will follow the beta values
						...
						if length(multiallstruct(idx)) > 0 %& length(multiallstruct(idx)) < 2 %add this if you can't find the duplicates
							...
							%% vector(bn) = multiallstruct(idx).BLLH(topo).(errvalue we want).model  %trying a more refined way of looping element into vector test and immediately switch to old way if failure	
							
							%perconerr(:,bn) = multiallstruct(idx).BLLH(topo).perconerr.(model);
							%precision(:,bn) = multiallstruct(idx).BLLH(topo).precision.(model);
							%recall(:,bn) 	= multiallstruct(idx).BLLH(topo).recall.(model);
							%fallout(:,bn)	= multiallstruct(idx).BLLH(topo).fallout.(model);
							%Jrrerr(:,bn)	= multiallstruct(idx).BLLH(topo).Jrrerr.(model);
							%Jsymrrerr(:,bn)		= multiallstruct(idx).BLLH(topo).Jsymrrerr.(model);
							%Javgsymrrerr(:,bn)	= multiallstruct(idx).BLLH(topo).Javgsymrrerr.(model);
							
							perconerr 		= [perconerr, multiallstruct(idx).BLLH(topo).perconerr.(model)];
							precision 		= [precision, multiallstruct(idx).BLLH(topo).precision.(model)];
							recall 			= [recall, multiallstruct(idx).BLLH(topo).recall.(model)];
							fallout			= [fallout, multiallstruct(idx).BLLH(topo).fallout.(model)];
							Jrrerr			= [Jrrerr, multiallstruct(idx).BLLH(topo).Jrrerr.(model)];
							Jsymrrerr		= [Jsymrrerr, multiallstruct(idx).BLLH(topo).Jsymrrerr.(model)];
							Javgsymrrerr	= [Javgsymrrerr, multiallstruct(idx).BLLH(topo).Javgsymrrerr.(model)];
							symperconerr 	= [symperconerr, multiallstruct(idx).BLLH(topo).symperconerr.(model)];
							TFS				= [TFS, multiallstruct(idx).BLLH(topo).TFS.(model)];
							TFR			 	= [TFR, multiallstruct(idx).BLLH(topo).TFR.(model)];
					...
					elseif comp == 4	% Relative recon err, for symmetry reconstruction
						...
						figstor(jax).fig(cn).c.(model).topo(topo).xxaxisvec =	uniqbeta;
					elseif comp == 5	% Relative recon err, for symmetry reconstruction
						figstor(jax).fig(cn).c.(model).topo(topo).yyaxisvec =  symperconerr;
						figstor(jax).fig(cn).c.(model).topo(topo).xxaxisvec = uniqbeta;
					elseif comp == 6	% PPC score
						figstor(jax).fig(cn).c.(model).topo(topo).yyaxisvec =  symperconerr;
						figstor(jax).fig(cn).c.(model).topo(topo).xxaxisvec = uniqbeta;
...
for jnx = 1:length(figstor) %total number of figure as decided by NxT
	...
	lgnd = legend(toponamevec,'Location','east');
	%set(lgnd,'color','none');
	...
	if comp == 1		% perconerr
		...
		[ax,h1]=suplabel([{'Average Error Rate for all Topologies'},{'For \beta values: 0.1, 0.2, 0.3, 0.4'}]);
	...
	elseif comp == 2	% AUROC
		...
		[ax,h1]=suplabel([{'Average "ROC score" for all Topologies'},{'For \beta values: 0.1, 0.2, 0.3, 0.4'}]);
		%[ax,h3]=suplabel(['N=',num2str(N),' | T=1E',num2str(log10(T))] ,'t');
		sgtitle({
	...
	elseif comp == 3 	% AUPRC
		...
		%[ax,h1]=suplabel('For \beta values: 0.1, 0.2, 0.3, 0.4');
		[ax,h1]=suplabel([{'Average "PRC score" for all Topologies'},{'For \beta values: 0.1, 0.2, 0.3, 0.4'}]);
		%[ax,h3]=suplabel(['N=',num2str(N),' | T=1E',num2str(log10(T))] ,'t');
		sgtitle({
	...
	elseif comp == 4	% Relative recon err, for symmetry reconstruction
		...
		%[ax,h1]=suplabel('For \beta values: 0.1, 0.2, 0.3, 0.4');
		[ax,h1]=suplabel([{'Average RRerr for all Topologies'},{'For \beta values: 0.1, 0.2, 0.3, 0.4'}]);
		%[ax,h3]=suplabel(['N=',num2str(N),' | T=1E',num2str(log10(T))] ,'t');
