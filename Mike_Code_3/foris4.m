...
addpath(genpath('E:\GitHub\Inv.Is.Models\Mike_Code_3'));
savepath

cd('E:\GitHub\Inv.Is.Models\Mike_Code_3\Warm Storage');  %Changes the working directory to where you want to store your output
...
%%%% Parameters
    ...
    Nvec =[80];					%Nvec = [50,100,150]; %Nvec = [50,100,300]; % Nvec = [100,200,300,400,500];
    ...
    betavec = [0.4,0.8,1.2]; 			%betavec = [0.4,0.9,1.4];
...
for Ti = 1:length(Tvec)
    ...
    for Ni = 1:length(Nvec)
        ...
        for  Ci = 1:length(coordvec)
            ...
            for Bi = 1:length(betavec)
                ...
                cd(topdir)
                betadir = ['Beta_',num2str(beta)]; % do I still need this?
                mkdir(cd,betadir);
                cd(betadir)
                        
               %name = ['N',num2str(N),'T1E',num2str(log10(T))];
               %name = ['St',num2str(Si),'Bt',num2str(Bi),'N',num2str(N),'T1E',num2str(log10(T))];
               name = ['St',num2str(Ci),'Bt',num2str(Bi),'N',num2str(N),'T',num2str(Ti)]; % have to keep in St for now even though it reps the coord no
               ...
               save([lowdir,'\',time(1:5),'parameters_',name,'_',time(6:12),'.mat'],'T','N','beta','topovec','jn','sprsvec','h_on','time','name','-v7.3');
                ...
                for trn = 1:jn
                    ...
                    for tp = skon:length(topovec) %topology = 1:6 % This doesn't work if you don't have a first place value of any sort in addition to the
                        topo = topovec{tp};
                        Adj = set_topology2(topo,N,c);
                        Jtopo = set_couplings2(couplings,beta,J0,sigJ,Adj);
                    ...
                    AllStruct.list(runs).Jcontru(trn).trial     = Adjset;
                    AllStruct.list(runs).Jtru(trn).trial        = Jtoposet;
                    AllStruct.list(runs).htru(trn).trial        = JHnorm.Htopo;
                    ...
                    AllStruct.list(runs).S(trn).trial = SstructNorm;
                    ...
                    AllStruct.list(runs).BLLH(trn).trial = LLH;
                ...
                OverStruct.AllStruct = AllStruct;
                save([topdir,'\',time(1:5),'OverStruct_',time(6:12),'.mat'],'OverStruct','-v7.3'); 
            end
    ...
    %OverStruct(Ti).AllStruct = AllStruct;
end
