% copy of model graphs 2
% Trying to fix the amassment of data and fix the Jstor
% not for the figures
% designed for newbllh.m
%


function Jstor = modelgraphs9(OverStruct)
	...
	for bt = 1:length(betavec)
		...
		for Nt = 1:length(Nvec)
			...
			for Tt = 1:length(Tvec)
				...
				for sprs = 1:length(sprsvec)
				%for sprs = 1:length(OverStruct)
					...
					for ft = 1:length(fn)
						...
						Jstor(topology).Tt					= topovec(topology);
						Jstor(topology).Topo(bt).BT				= betavec(bt);
						Jstor(topology).Topo(bt).beta(Nt).Nt 	=  N;
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).Tt			= T;
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Sparsity = sprsvec(sprs);
						
	
						
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.couplings		=  	OverStruct(ali).AllStruct.(name).couplings;
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.c				=  	OverStruct(ali).AllStruct.(name).c;
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.Jcontru		=  	OverStruct(ali).AllStruct.(name).Jcontru(topology);
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.Jtru			=  	OverStruct(ali).AllStruct.(name).Jtru(topology);
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.htru			=  	OverStruct(ali).AllStruct.(name).htru(1,:); %OverStruct(sprs).AllStruct.(name).htru(topology,:); % This doesn't work for as long as we only have one h
						Jstor(topology).Topo(bt).beta(Nt).N(Tt).T(sprs).Spars.S				=  	OverStruct(ali).AllStruct.(name).S(topology);	
					...
					%end
						...
						for mt = 1:length(models)
							...
							disp(['Topo',num2str(topovec{topology}),'_Bt',num2str(betavec(bt)),'_N',num2str(Nvec(Nt)),'_T1E',num2str(log10(Tvec(Tt))),'_Sprs',num2str(sprsvec(sprs)),'_model_',num2str(models{mt})]);
	
							model = models{mt};
	
							Jcontru			 =		OverStruct(ali).AllStruct.(name).Jcontru{topology};
							Jrecon		 	 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).Jrecon.(model);
							Jcon 			 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).Jcon.(model);
							hrecon			 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).hrecon.(model);
							...
							perconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).perconerr.(model);
							tpconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).tpconerr.(model);
							tnconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).tnconerr.(model);
							fpconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).fpconerr.(model);
							fnconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).fnconerr.(model);
							totconerr		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).totconerr.(model);
							symrate			 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).symrate.(model);
							asymrate		 =  	OverStruct(ali).AllStruct.(name).BLLH(topology).asymrate.(model);
							fallout			=  		OverStruct(ali).AllStruct.(name).BLLH(topology).fallout.(model);
							recall			=  		OverStruct(ali).AllStruct.(name).BLLH(topology).recall.(model);
							precision		=  		OverStruct(ali).AllStruct.(name).BLLH(topology).precision.(model);
